...
// Pour les dégressives : sets × paliers. Ex: 2 séries dégressive en 3 = 6 lignes
...
// ─── Input brutaliste ─────────────────────────────────────────────────────────
const Input = ({ style={}, onFocus, …rest }) => (
<input
{…rest}
onFocus={e => {
// Sélectionne tout le texte au tap → on tape directement par-dessus, pas de curseur à placer
e.target.select();
onFocus && onFocus(e);
}}
style={{
...
// Calcul du label selon type d’exercice
let rowLabel;
if (ex.degressive) {
const serieNum  = Math.floor(idx / ex.degressive) + 1;
const palierNum = (idx % ex.degressive) + 1;
const isPrincipal = palierNum === 1;
rowLabel = isPrincipal
? `SÉRIE ${pad2(serieNum)}`
: `↓ PALIER ${pad2(palierNum)}`;
} else {
rowLabel = `SET ${pad2(idx+1)}`;
}

// IDs uniques pour l’auto-avance
...
const advance = nextId => e => {
if (e.key === “Enter” || e.key === “Next”) { e.preventDefault(); if (nextId) focus(nextId); }
};

return (
<div style={{
padding:“18px 0”, borderBottom:`1px solid ${T.hairline}`,
// Fond légèrement différent sur les paliers de drop
background: ex.degressive && (idx % ex.degressive) > 0 ? T.surface2+“66” : “transparent”
}}>
{/* Label */}
<div style={{
fontFamily:FF_MONO, fontSize:11, fontWeight:700,
color: ex.degressive && (idx % ex.degressive) > 0 ? T.accent : T.textMid,
letterSpacing:“0.16em”, marginBottom:12
}}>
{rowLabel}
{ex.degressive && (idx % ex.degressive) > 0 &&
<span style={{fontFamily:FF_SANS,fontSize:10,fontWeight:400,color:T.textDim,marginLeft:10,letterSpacing:“0.04em”}}>
sans repos · baisse 20-25%
</span>
}
</div>
  ...
  <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
    <div style={{flex:1,minWidth:0}}>
      {ex.superset ? (
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div>
            <div style={{...labelStyle,marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
              <span style={{background:T.accent,color:T.bg,padding:"2px 7px",fontSize:9,fontWeight:700,letterSpacing:"0.12em"}}>A</span>
              KG × REPS
            </div>
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              <Input id={idWA} placeholder="—" inputMode="decimal" value={data.weightA||""} onChange={e=>onUpdate({weightA:e.target.value})} onKeyDown={advance(idRA)} disabled={done} style={{flex:1}}/>
              <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
              <Input id={idRA} placeholder="—" inputMode="numeric" value={data.repsA||""} onChange={e=>onUpdate({repsA:e.target.value})} onKeyDown={advance(idWB)} disabled={done} style={{flex:1}}/>
            </div>
            {suggestion?.wA>0 && !done && (
              <button onClick={()=>onUpdate({weightA:String(suggestion.wA)})} style={{background:"transparent",border:"none",cursor:"pointer",padding:"8px 0 0",textAlign:"left",display:"block"}}>
          ...
          <div>
            <div style={{...labelStyle,marginBottom:8,display:"flex",alignItems:"center",gap:10}}>
              <span style={{background:T.surface3,color:T.textMid,padding:"2px 7px",fontSize:9,fontWeight:700,letterSpacing:"0.12em"}}>B</span>
              KG × REPS
            </div>
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              <Input id={idWB} placeholder="—" inputMode="decimal" value={data.weightB||""} onChange={e=>onUpdate({weightB:e.target.value})} onKeyDown={advance(idRB)} disabled={done} style={{flex:1}}/>
              <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
              <Input id={idRB} placeholder="—" inputMode="numeric" value={data.repsB||""} onChange={e=>onUpdate({repsB:e.target.value})} onKeyDown={advance(nextWA)} disabled={done} style={{flex:1}}/>
            </div>
            {suggestion?.wB>0 && !done && (
              <button onClick={()=>onUpdate({weightB:String(suggestion.wB)})} style={{background:"transparent",border:"none",cursor:"pointer",padding:"8px 0 0",textAlign:"left",display:"block"}}>
        ...
        <div>
          <div style={{...labelStyle,marginBottom:8}}>KG × REPS{targetReps?` · CIBLE ${targetReps}`:""}</div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            <Input id={idW} placeholder="—" inputMode="decimal" value={data.weight||""} onChange={e=>onUpdate({weight:e.target.value})} onKeyDown={advance(idR)} disabled={done} style={{flex:1}}/>
            <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
            <Input id={idR} placeholder={targetReps||"—"} inputMode="numeric" value={data.reps||""} onChange={e=>onUpdate({reps:e.target.value})} onKeyDown={advance(nextW)} disabled={done} style={{flex:1}}/>
          </div>
          {suggestion?.w>0 && !done && (
            <button onClick={()=>onUpdate({weight:String(suggestion.w)})} style={{background:"transparent",border:"none",cursor:"pointer",padding:"8px 0 0",textAlign:"left",display:"block"}}>
    ...
    <button
      ...
      style={{
        ...
        flexShrink:0, alignSelf:ex.superset?"flex-end":"center", padding:0, borderRadius:0, transition:"background 0.15s"
      }}>
...
return (
<style>{`@import url('${FONTS_URL}'); *,*::before,*::after { box-sizing: border-box; } html { overflow-x: hidden; overscroll-behavior-x: none; width: 100%; max-width: 100vw; } html,body,#root { margin:0; padding:0; background:${T.bg}; color:${T.text}; font-family:${FF_SANS}; -webkit-font-smoothing:antialiased; -moz-osx-font-smoothing:grayscale; text-rendering:optimizeLegibility; } body { overscroll-behavior: none; overflow-x: hidden; width: 100%; max-width: 100vw; position: relative; } #root { overflow-x: hidden; width: 100%; max-width: 100vw; } input,textarea,button,select { font-family: inherit; color: inherit; max-width: 100%; } input:focus,textarea:focus,select:focus { border-color: ${T.accent} !important; outline: none; } button { font-family: inherit; -webkit-tap-highlight-color: transparent; } button:active:not(:disabled) { transform: scale(0.98); } input[type=number]::-webkit-inner-spin-button, input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; } input[type=number] { -moz-appearance: textfield; } input::placeholder, textarea::placeholder { color: ${T.textFaint}; } img, svg, video { max-width: 100%; height: auto; } @keyframes fadeIn { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: translateY(0); } } @keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.55; } } .fade-in { animation: fadeIn 0.32s cubic-bezier(0.16,1,0.3,1); } ::selection { background: ${T.accent}; color: ${T.bg}; } ::-webkit-scrollbar { width: 4px; height: 4px; } ::-webkit-scrollbar-thumb { background: ${T.surface3}; } ::-webkit-scrollbar-track { background: transparent; } select { appearance: none; -webkit-appearance: none; background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='none' stroke='%238A8A85' stroke-width='1'><path d='M6 9l6 6 6-6'/></svg>"); background-repeat: no-repeat; background-position: right 6px center; padding-right: 26px !important; }`}</style>
);
}