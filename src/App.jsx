...
const Display = ({ children, size=40, weight=800, style={} }) => (
  <span style={{fontFamily:FF_DISPLAY, fontSize:size, fontWeight:weight, letterSpacing:"-0.02em", textTransform:"uppercase", color:T.text, lineHeight:0.92, display:"block", ...style}}>{children}</span>
);
...
const Modal = ({ open, onClose, children, title, code }) => {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:T.bg,zIndex:2000,display:"flex",flexDirection:"column",animation:"fadeIn 0.18s"}}>
      <div style={{maxWidth:520,margin:"0 auto",padding:"20px 20px 40px",width:"100%",boxSizing:"border-box",flex:1,overflowY:"auto"}}>
...
function RestBar({ timer, onSkip, onAdd }) {
  ...
  return (
    <div style={{position:"fixed",top:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:520,zIndex:1000,background:T.bg,borderBottom:`1px solid ${urgent?T.accent:T.hairlineMid}`,padding:"12px 20px 10px",boxSizing:"border-box"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        <Label color={urgent?T.accentBright:T.textMid}>REPOS {urgent && "· FIN"}</Label>
        <div style={{display:"flex",gap:14}}>
          <button onClick={onAdd} style={{background:"transparent",border:"none",color:T.textMid,cursor:"pointer",padding:0,fontFamily:FF_MONO,fontSize:10,fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase"}}>+30S</button>
          <button onClick={onSkip} style={{background:"transparent",border:"none",color:T.textMid,cursor:"pointer",padding:0,fontFamily:FF_MONO,fontSize:10,fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase"}}>PASSER</button>
        </div>
      </div>
      <div style={{fontFamily:FF_DISPLAY,fontSize:urgent?44:36,fontWeight:700,color:urgent?T.accentBright:T.text,letterSpacing:"-0.03em",fontVariantNumeric:"tabular-nums",lineHeight:0.9,marginBottom:8,transition:"color 0.2s, font-size 0.2s"}}>
        {fmtClock(timer.remaining)}
      </div>
      <div style={{height:2,background:T.surface2,position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,width:`${pct}%`,background:urgent?T.accent:T.text,transition:"width 1s linear"}}/>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{padding:"20px 0 36px",borderBottom:`1px solid ${T.hairlineMid}`,marginBottom:36}}>
        ...
        <Display size={64} weight={900} style={{marginBottom:6}}>PROCHAINE</Display>
        <Display size={64} weight={900} style={{color:T.accent}}>SÉANCE</Display>
      </div>
            ...
            <button key={sess.key}
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:18}}>
                <div style={{flexShrink:0,minWidth:56}}>
                  ...
                  <Display size={54} weight={900} style={{color:suggested?T.accent:T.text,letterSpacing:"-0.04em"}}>{sess.label}</Display>
                </div>
                <div style={{flex:1,minWidth:0,paddingTop:2}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,marginBottom:10}}>
                    <Label color={T.text}>{sess.title}</Label>
                    <Ico d={IC.arrowR} s={18} c={suggested?T.accent:T.textMid}/>
                  </div>
                  ...
                  <div style={{display:"flex",gap:18,alignItems:"center"}}>
                    <span style={{fontFamily:FF_MONO,fontSize:9,fontWeight:500,color:T.textDim,letterSpacing:"0.12em",textTransform:"uppercase"}}>
                      {last ? `DERN. ${fmtAgo(last.date)}` : "JAMAIS FAIT"}
                    </span>
                    <span style={{fontFamily:FF_MONO,fontSize:9,fontWeight:500,color:T.textDim,letterSpacing:"0.12em",textTransform:"uppercase"}}>
                      × {pad3(count)}
                    </span>
                    {isActive && <span style={{fontFamily:FF_MONO,fontSize:9,fontWeight:600,color:T.accent,letterSpacing:"0.12em",textTransform:"uppercase"}}>● EN COURS</span>}
                    {suggested && !isActive && <span style={{fontFamily:FF_MONO,fontSize:9,fontWeight:600,color:T.accent,letterSpacing:"0.12em",textTransform:"uppercase"}}>· SUGGÉRÉE</span>}
                  </div>
                </div>
              </div>
            </button>
      ...
      <button onClick={()=>{haptic();setOpen(o=>!o);}} style={{width:"100%",background:"transparent",border:"none",padding:"22px 0 20px",cursor:"pointer",textAlign:"left",display:"block"}}>
        ...
        {ex.superset ? (
          <div>
            ...
            <Display size={22} weight={700} style={{marginBottom:12}}>+ {ex.nameB}</Display>
          </div>
        ) : (
          <Display size={26} weight={700} style={{marginBottom:12}}>{ex.name}</Display>
        )}

        <div style={{display:"flex",gap:20,alignItems:"center",flexWrap:"wrap"}}>
          <Label color={T.textMid}>{volLabel}</Label>
          {lastMax && (
            <Label color={T.textMid}>
              DERN. {lastMax.isSuperset?`${lastMax.wA||"—"}·${lastMax.wB||"—"}`:lastMax.w}KG
            </Label>
          )}
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{padding:"8px 0 32px",borderBottom:`1px solid ${T.hairlineMid}`,marginBottom:28}}>
        ...
        <Display size={72} weight={900} style={{letterSpacing:"-0.04em",marginBottom:8}}>{sess.label}</Display>
      ...
      {/* Interrompre */}
      <button onClick={()=>setStopModal(true)} style={{background:"transparent",border:"none",padding:"0 0 28px",cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
        <Ico d={IC.close} s={12} c={T.textDim}/>
        <Label color={T.textDim}>INTERROMPRE LA SÉANCE</Label>
      </button>
  ...
  return (
    <div className="fade-in">
      <div style={{padding:"20px 0 32px",borderBottom:`1px solid ${T.hairlineMid}`,marginBottom:36}}>
        ...
        <Display size={64} weight={900} style={{marginBottom:6}}>PROGRES-</Display>
        <Display size={64} weight={900}>SION</Display>
      </div>
  ...
  return (
    <div className="fade-in">
      <div style={{padding:"20px 0 32px",borderBottom:`1px solid ${T.hairlineMid}`,marginBottom:36}}>
        ...
        <Display size={64} weight={900} style={{marginBottom:6}}>RÉGLAGES</Display>
      </div>
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:T.bg,position:"relative"}}>
        ...
        <div style={{maxWidth:520,margin:"0 auto",padding:"16px 20px 120px",paddingTop:timer.active?110:16,position:"relative",zIndex:1,transition:"padding-top 0.3s"}}>
          {/* Header wordmark */}
