...
function SetRow({ idx, data, ex, suggestion, targetReps, onUpdate, onComplete }) {
  ...
  return (
    <div style={{padding:"20px 0",borderBottom:`1px solid ${T.hairline}`}}>
      {/* Numéro de série */}
      <div style={{fontFamily:FF_MONO,fontSize:11,fontWeight:600,color:T.textMid,letterSpacing:"0.16em",marginBottom:12}}>
        SET {pad2(idx+1)}
      </div>
      ...
      <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
        <div style={{flex:1,minWidth:0}}>
          ...
          {ex.superset ? (
            /* Superset : A en haut, B en bas, inputs pleine largeur */
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <Input placeholder="—" inputMode="decimal" value={data.weightA||""} onChange={e=>onUpdate({weightA:e.target.value})} disabled={done} style={{flex:1}}/>
                  <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
                  <Input placeholder="—" inputMode="numeric" value={data.repsA||""} onChange={e=>onUpdate({repsA:e.target.value})} disabled={done} style={{flex:1}}/>
                </div>
              ...
              <div>
                ...
                <div style={{display:"flex",gap:10,alignItems:"center"}}>
                  <Input placeholder="—" inputMode="decimal" value={data.weightB||""} onChange={e=>onUpdate({weightB:e.target.value})} disabled={done} style={{flex:1}}/>
                  <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
                  <Input placeholder="—" inputMode="numeric" value={data.repsB||""} onChange={e=>onUpdate({repsB:e.target.value})} disabled={done} style={{flex:1}}/>
                </div>
            ...
            /* Exercice normal : 1 ligne kg × reps */
            <div>
              <div style={{...labelStyle,marginBottom:8}}>KG × REPS{targetReps?` · CIBLE ${targetReps}`:""}</div>
              <div style={{display:"flex",gap:10,alignItems:"center"}}>
                <Input placeholder="—" inputMode="decimal" value={data.weight||""} onChange={e=>onUpdate({weight:e.target.value})} disabled={done} style={{flex:1}}/>
                <span style={{color:T.textMid,fontFamily:FF_MONO,fontSize:16,fontWeight:600,flexShrink:0}}>×</span>
                <Input placeholder={targetReps||"—"} inputMode="numeric" value={data.reps||""} onChange={e=>onUpdate({reps:e.target.value})} disabled={done} style={{flex:1}}/>
              </div>
              {suggestion?.w>0 && !done && (
